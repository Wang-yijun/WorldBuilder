...
#include "world_builder/features/continental_plate_models/grains/uniform_distribution_deflected.h"
...
namespace WorldBuilder
{
  ...
  namespace Features
  {
    namespace ContinentalPlateModels
    {
      namespace Grains
      {
        UniformDistributionDeflected::UniformDistributionDeflected(WorldBuilder::World *world_)
          :
          min_depth(NaN::DSNAN),
          max_depth(NaN::DSNAN)
        ...
        {
          this->world = world_;
          this->name = "uniform distribution deflected";
        }

        UniformDistributionDeflected::~UniformDistributionDeflected()
          = default;

        void
        UniformDistributionDeflected::declare_entries(Parameters &prm, const std::string & /*unused*/)
        {
        ...
        void
        UniformDistributionDeflected::parse_entries(Parameters &prm, const std::vector<Point<2>> &coordinates)
        {
        ...
        WB_REGISTER_FEATURE_CONTINENTAL_PLATE_GRAINS_MODEL(UniformDistributionDeflected, uniform distribution deflected)
      } // namespace Grains
    } // namespace ContinentalPlateModels
  } // namespace Features
} // namespace WorldBuilder
